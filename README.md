...
## 8. Raw FTICRMS UpSet 交集图：`upset`

用途：从 Raw FT-ICR MS 分子式表格中绘制 YL、OL、ML 三组样品的 Formula 交集 UpSet 图。该命令用于复现当前 Raw_FTICRMS_UpSet 图的排版风格，包括上方交集柱状图、下方组合矩阵、左侧 formula count 横条、独立的 YL/OL/ML 行标签以及可编辑 PDF/SVG 输出。

典型调用方式：

```text
调用 upset，读取 FTICRMS/Raw 文件夹，绘制 YL、OL、ML 的 Formula 交集图，并输出 PDF、SVG、PNG、TIFF 和 source data。
```
...
- 下方矩阵面板必须按上方柱状图的坐标轴空间对齐，矩阵左边界与上方 y 轴线保持同一竖直位置。
- `YL/OL/ML` 行标签使用独立窄列，不参与矩阵坐标轴空间计算，避免破坏上下对齐。
- 灰色 inactive points 先绘制，蓝色连接线和蓝色 active points 后绘制，确保蓝色线条在上层。
- 可见坐标轴线和刻度线使用黑色。
- 左侧 formula count 横条靠近行标签列。
- 默认图尺寸为 `7.2 × 4.6 in`，默认样品顺序为 `YL,OL,ML`。
- 输出 PDF/SVG 为可编辑矢量版本，PNG/TIFF 用于预览和投稿检查。
